...
import { Clock, AlarmClock, Timer, BrainCircuit, ListTodo } from 'lucide-react';
...
export const Nav: React.FC<NavProps> = ({ currentMode, setMode }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 p-4 z-50">
      <div className="max-w-xl mx-auto bg-surfaceContainer/90 backdrop-blur-xl rounded-[2rem] shadow-lg p-2 flex justify-between items-center border border-white/5">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] font-medium mt-1 absolute bottom-2 opacity-0 transition-opacity duration-300 ${isActive ? 'opacity-100' : ''}`}>
                {item.label}
              </span>
            </button>
          );
